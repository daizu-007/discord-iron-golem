...
// 必要なライブラリを読み込む
import {Client, Events, GatewayIntentBits, type Role, type TextChannel} from 'discord.js';
import toml from 'toml';
import fs from 'fs';
import * as keywordVerifier from './modules/keywordVerifier.js';
...
// ログチャンネルを保存する変数
let log_channel: TextChannel | null = null;
...
client.once(Events.ClientReady, readyClient => {
    ...
    if (guild) {
        role = guild.roles.cache.get(role_id) ?? null;
        log_channel = guild.channels.cache.get(log_channel_id) as TextChannel | undefined ?? null;
    }
    if (log_channel == null) {
